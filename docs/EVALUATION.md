...
Each entry specifies a `query` string.
...
Target reduction range: **86–99%**. If reduction is below 86%, the candidate set is too large and retrieval quality may degrade. If reduction exceeds 99%, the keyword filter is too aggressive and relevant papers may be excluded — the pipeline falls back to the full corpus automatically when candidate count drops below `min_prefilter_candidates` (default 500).
...
|---|---|---|
| No keyword prefilter | `--no-qwen` | Skips Qwen extraction; uses the full corpus |
