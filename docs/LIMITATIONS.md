...
**Category assignment**
arXiv categories come from author self-reporting and are stored on each `Paper` record. Since issue #10, `pipeline.run(categories=[...])` (and the Streamlit "Limit to arXiv categories" multiselect) restricts results to papers cross-listed in at least one of the selected categories via `candidate_ids_matching_categories`, intersected with the keyword/dense/BM25 candidate sets. Category assignment itself is still author-self-reported: a paper may be cross-listed in multiple categories, or placed in a category that does not fully reflect its content, so the filter is only as accurate as that self-reporting.
...
To mitigate this for short queries (issue #14), `pipeline.run()` treats any query under `SHORT_QUERY_WORD_THRESHOLD` (default 4) words as "short/ambiguous" and automatically runs a second dense search against the raw query text, folding both dense rankings into RRF fusion — reusing the dual raw-query/HyDE machinery that `hyde_ablation=True` added for debugging (issue #2). This lets results that rank well on literal wording surface even if the generated abstract drifts off-target; `response.trace.short_query_dual_dense` reports whether this path was taken. Normal-length queries (>= 4 words) keep the existing HyDE-only default to avoid the extra latency. The 4-word threshold is a simple heuristic, not a learned classifier, and may misclassify some short-but-precise or longer-but-vague queries — it hasn't been tuned against retrieval-quality metrics.
...
After the issue #10 re-harvest grew the corpus to 3.07M papers, `RetrievalTrace.dense_latency_seconds` for the *first* query in a freshly-started process is ~60-70s (e.g. `dense_latency_seconds: 71.235` of `total_latency_seconds: 90.825`), dominating total latency. Profiling isolated this to ChromaDB lazily loading its on-disk HNSW segment for the 3.07M-vector collection on the first `collection.query()` call — embedding and a *warm* query are both fast in isolation (<0.2s). This is a one-time per-process cost, not per-query: `scripts/run_query.py` (a fresh process per invocation) pays it every call, while the Streamlit app caches the pipeline in `st.session_state` and pays it once per session. There is currently no warm-up step during pipeline initialization.
