...
After the issue #10 re-harvest grew the corpus to 3.07M papers, `RetrievalTrace.dense_latency_seconds` for the *first* query in a freshly-started process is ~60-70s, dominating total query latency (e.g. a 5-result CLI query: `dense_latency_seconds: 71.235`, `total_latency_seconds: 90.825`). Profiling (`scripts/_debug_dense_timing*.py`, not checked in) isolated this to ChromaDB lazily loading its on-disk HNSW segment for the 3.07M-vector collection into memory on the first `collection.count()`/`collection.query()` call after `DenseRetriever.__init__` — `SentenceTransformer.encode()` (~0.02-0.05s) and a *warm* `collection.query()` (0.03s for `n_results=5`, 0.16s for `n_results=200`) are both fast in isolation. This is a one-time per-process cost, not a per-query algorithmic one: `scripts/run_query.py` (a fresh process per invocation) pays it on every call, while the Streamlit app caches `RagLiteraturePipeline` in `st.session_state` and so should only pay it once per session, on the first search. There is currently no warm-up step that pre-triggers this load during pipeline initialization.
